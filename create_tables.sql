...
/*f*/select model from (select model, max(speed) from (select * from (select * from (PC natural join Product)) as t1 where ram=(select min(ram) from PC)) as t2) as t3;
	    ...
	    /*this works and shouldn't*/select model from PC natural join Product where ram=(select min(ram) from PC) and speed=(select max(speed) from PC);
	    /*why doesn't this work?*/select model from (select model,max(speed) from PC natural join Product as t1 where ram = (select min(ram) from PC));
	    /*why can't I refer to t3 this way?*/
	    select model, speed from (select model, speed from (select * from PC natural join Product as t1) as t2 where ram=(select min(ram) from PC)) as t3 where speed = (select max(speed) from t2);
